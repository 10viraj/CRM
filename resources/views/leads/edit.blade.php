...
@section('title', 'Edit Lead - SmartCRM')
...
<div class="mb-8">
    ...
    <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Edit Lead</h1>
    <p class="text-slate-500 mt-1 text-sm font-medium">Update the details for this lead.</p>
</div>

<div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden max-w-4xl">
    <form action="{{ route('leads.update', $lead) }}" method="POST" class="p-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
                ...
                <input type="text" name="first_name" id="first_name" value="{{ old('first_name', $lead->first_name) }}" class="w-full rounded-lg border-slate-300 border px-4 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition @error('first_name') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror">
            ...
            <div>
                ...
                <input type="text" name="last_name" id="last_name" value="{{ old('last_name', $lead->last_name) }}" class="w-full rounded-lg border-slate-300 border px-4 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition @error('last_name') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror">
            ...
            <div>
                ...
                <input type="email" name="email" id="email" value="{{ old('email', $lead->email) }}" class="w-full rounded-lg border-slate-300 border px-4 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition @error('email') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror" placeholder="contact@example.com">
                @error('email') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
                <label for="phone" class="block text-sm font-medium text-slate-700 mb-1">Phone Number</label>
                <input type="text" name="phone" id="phone" value="{{ old('phone', $lead->phone) }}" class="w-full rounded-lg border-slate-300 border px-4 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition @error('phone') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror" placeholder="(555) 123-4567">
                @error('phone') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div class="md:col-span-2">
                <label for="company" class="block text-sm font-medium text-slate-700 mb-1">Company</label>
                <input type="text" name="company" id="company" value="{{ old('company', $lead->company) }}" class="w-full rounded-lg border-slate-300 border px-4 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition @error('company') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div>
                ...
                <select name="lead_status_id" id="lead_status_id" class="w-full rounded-lg border-slate-300 border px-4 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition bg-white @error('lead_status_id') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror">
                    ...
                    @foreach($statuses as $status)
                        <option value="{{ $status->id }}" {{ old('lead_status_id', $lead->lead_status_id) == $status->id ? 'selected' : '' }}>{{ $status->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="number" name="score" id="score" min="0" max="100" value="{{ old('score', $lead->score) }}" class="w-full rounded-lg border-slate-300 border px-4 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition @error('score') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror">
            ...
            <div class="md:col-span-3">
                ...
                <textarea name="notes" id="notes" rows="4" class="w-full rounded-lg border-slate-300 border px-4 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition @error('notes') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror" placeholder="Initial context or qualification notes...">{{ old('notes', $lead->notes) }}</textarea>
        ...
        <div class="flex justify-end space-x-3 pt-6 border-t border-slate-100">
            ...
            <button type="submit" class="px-5 py-2.5 rounded-lg text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm transition">Update Lead</button>
        </div>
    </form>
</div>
@endsection
