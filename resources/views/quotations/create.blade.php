...
@section('title', 'Create Quotation - SmartCRM')

@section('content')
<div class="mb-8">
    <a href="{{ route('quotations.index') }}" class="text-sm text-indigo-600 hover:text-indigo-800 flex items-center mb-4">
        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
        Back to Quotations
    </a>
    <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Create Quotation</h1>
</div>

<form action="{{ route('quotations.store') }}" method="POST" x-data="quotationCalculator()" class="space-y-6">
    @csrf
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="p-8">
            <h2 class="text-lg font-semibold text-slate-800 mb-6 border-b border-slate-100 pb-2">Quotation Details</h2>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Quote Number</label>
                    <input type="text" name="quote_number" value="{{ old('quote_number', $nextNumber) }}" class="w-full rounded-lg border-slate-300 border px-4 py-2 text-sm bg-slate-50 focus:outline-none" readonly>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="expiry_date" value="{{ old('expiry_date', date('Y-m-d', strtotime('+30 days'))) }}" class="w-full rounded-lg border-slate-300 border px-4 py-2 text-sm focus:border-indigo-500 outline-none">
                </div>
                <div>
        ...
        <div class="p-8">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start pt-4 border-t border-slate-200">
                <div class="w-full md:w-1/2 mb-6 md:mb-0 pr-0 md:pr-8">
                    ...
                    <textarea name="notes" rows="4" class="w-full rounded-lg border-slate-300 border px-4 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition" placeholder="Terms and conditions, payment details, etc...">{{ old('notes') }}</textarea>
                </div>
            ...
            <div class="flex justify-end space-x-3 mt-8 pt-6 border-t border-slate-100">
                <a href="{{ route('quotations.index') }}" class="px-5 py-2.5 rounded-lg text-sm font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 transition">Cancel</a>
                <button type="submit" class="px-5 py-2.5 rounded-lg text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm transition">Save Quotation</button>
            </div>
        </div>
    </div>
</form>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('quotationCalculator', () => ({
            items: [
            ...
            updatePrice(item) {
                // Get the selected option's data-price attribute using standard DOM because Alpine binds product_id to the value
                if(item.product_id) {
                    ...
                    if(selectEl && selectEl.selectedOptions[0]) {
                        ...
                        if(price) {
                            ...
                            // Auto calc 10% tax for demo purposes if tax is 0
